...
export const getCartItems = (id) => {
  return (dispatch) => {
    return axios
      .get(`http://localhost:8080/cart-items/cartID/${id}`)
      .then((res) => {
...
export const addCartItems = (data) => {
  return (dispatch) => {
    return axios
      .post(`http://localhost:8080/cart-items`, data)
      .then(() => {
...
export const editCartItems = (data) => {
  return (dispatch) => {
    return axios({
      method: 'put',
      url: `http://localhost:8080/cart-items/${data.id}`,
      data: { ...data },
    })
      .then(() => {
...
export const deleteCartItems = (productid) => {
  return (dispatch) => {
    return axios({
      method: 'delete',
      url: `http://localhost:8080/cart-items/${productid}`,
    })
      .then(() => {
