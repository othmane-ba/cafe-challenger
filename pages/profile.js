...
export default function profile2() {
  ...
  useEffect(() => {
    ...
    if (!isEmpty(client)) {
      console.log(client);
      const adresse = isEmpty(client.address) ? "" : client.address;
      const telephone = isEmpty(client.phone_number) ? "" : client.phone_number;
      setInfo({
        ...
        <div className="grid grid-cols-1 gap-7 border border-gray-300 shadow-lg p-6 rounded-md max-w-full w-full">
          <div className="grid grid-cols-6 md:grid-cols-1 gap-2 py-5">
            ...
            <div className="col-span-5">
              {!isEmpty(client) && !isEmpty(info) && (
                <Profile_details info={info}/>
              )}
            </div>
          </div>
